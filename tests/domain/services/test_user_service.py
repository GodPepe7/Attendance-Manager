...
from tests.conftest import engine, tables, db_session_with_data, db_session_no_data


class TestUserService:
    @pytest.fixture
    def user_service_no_users(self, db_session_no_data):
        session = db_session_no_data
        repo = UserRepository(session)
        return session, UserService(repo)

    @pytest.fixture
    def user_service_with_users(self, db_session_with_data):
        session = db_session_with_data
        repo = UserRepository(session)
        return session, UserService(repo)

    def test_get_all_users_empty(self, user_service_no_users):
        session, user_service = user_service_no_users

        fetchedUsers = user_service.get_all()

        assert len(fetchedUsers) == 0
